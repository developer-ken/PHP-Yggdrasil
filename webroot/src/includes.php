<?php
ini_set("display_errors","Off");

//mysql配置//
//mysql Configuration//
$mysql_server = "192.168.1.7";
$mysql_port = 3306;
$mysql_user = "minecraftauth";
$mysql_passwd = "Platium!Calacidunma?";
$mysql_db = "minecraft";

//站点配置//
//site configuration//
$_meta = array(
    "serverName"=>"Yggdrasil的一个实现",//这里可以改成你希望的名字 change this to the authserver name you want.
    "implementationName"=>"php-mcpc-authserver",//不要修改 DO NOT TOUCH THIS
    "implementationVersion"=>"1.1"//不要修改 DO NOT TOUCH THIS
);

$_skin_domains = array(
    ".mcserver.fun"//这里改成你皮肤链接所用的域名 change this to the domain of your skinserver
);
$ConsultMojangSessionServer = true;//是否允许使用正版账号的玩家直接进入服务器 to allow authentic players to login

/*
* 使用正版账号的玩家不需要安装authlib-injector
* 在开启(true)上方选项后，当MC服务器向本程序查询session时，如果数据库中无有效session
* 则本程序会从Mojang的SessionServer查询。如果找到则返回正版Session
*/

//加密配置//
//Encrypt Configuration//
//MUST CHANGE THIS 必须修改//
$encypt_privkey = "";//加密所需私钥
$encypt_publkey = "";//加密所需公钥

//!! 不要修改下面的代码!! //
//!! TOUCH NOTHING BELOW !!//
spl_autoload_register("_autoload");

function _autoload($classname){
    require_once $_SERVER['DOCUMENT_ROOT'] . "/src/classes/" . strtolower($classname) . ".php";
}

$db = new DataBase();
?>
